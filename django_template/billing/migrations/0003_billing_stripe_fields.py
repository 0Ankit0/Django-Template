...
def normalize_billing_data(apps, schema_editor):
    ...
    Product.objects.filter(provider_product_id="").update(provider_product_id=None)
    Price.objects.filter(provider_price_id="").update(provider_price_id=None)
    BillingCustomer.objects.filter(provider_customer_id="").update(provider_customer_id=None)
    Subscription.objects.filter(provider_subscription_id="").update(provider_subscription_id=None)
    Subscription.objects.filter(provider_customer_id="").update(provider_customer_id=None)
    Payment.objects.filter(provider_payment_id="").update(provider_payment_id=None)
    Payment.objects.filter(provider_invoice_id="").update(provider_invoice_id=None)
    Invoice.objects.filter(provider_invoice_id="").update(provider_invoice_id=None)
    CheckoutSession.objects.filter(provider_session_id="").update(provider_session_id=None)
