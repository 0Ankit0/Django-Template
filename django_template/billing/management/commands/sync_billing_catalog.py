    ...
    def handle(self, *args, **options):
        for product in Product.objects.all().order_by("pk"):
            ...
            for price in Price.objects.filter(product=product, provider_price_id="").order_by("pk"):
                payload = {"product": product.provider_product_id, "unit_amount": price.amount, "currency": price.currency.lower(), "active": price.active, "metadata": {"local_price_id": str(price.pk)}}
