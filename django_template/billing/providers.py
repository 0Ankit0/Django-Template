...
def create_khalti_checkout(request, price) -> GatewayResult:
    ...
    if price.is_recurring:
        raise ValueError("Khalti is configured for one-time checkout in this template; use Stripe for recurring subscriptions.")
...
def create_esewa_checkout(request, price) -> GatewayResult:
    ...
    if price.is_recurring:
        raise ValueError("eSewa is configured for one-time checkout in this template; use Stripe for recurring subscriptions.")
...
def esewa_status(transaction_uuid: str, total_amount: str) -> dict:
    base = "https://uat.esewa.com.np/api/epay/transaction/status/" if settings.ESEWA_ENVIRONMENT == "sandbox" else "https://epay.esewa.com.np/api/epay/transaction/status/"
    query = urlencode({"product_code": settings.ESEWA_PRODUCT_CODE, "total_amount": total_amount, "transaction_uuid": transaction_uuid})
