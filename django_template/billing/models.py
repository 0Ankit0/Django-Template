...
class Price(models.Model):
    ...
    def is_recurring(self) -> bool:
        return self.interval not in {self.Interval.ONE_TIME, self.Interval.DAY, self.Interval.WEEK}

    @property
    def is_expiring_purchase(self) -> bool:
        return self.interval != self.Interval.ONE_TIME and not self.is_recurring
    ...
    def __str__(self) -> str:
        suffix = "" if self.interval == self.Interval.ONE_TIME else f" / {self.interval_count} {self.interval}"
...
class BillingCustomer(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(fields=["tenant", "provider"], name="billing_customer_tenant_provider_unique"),
            models.UniqueConstraint(fields=["provider", "provider_customer_id"], name="billing_customer_provider_id_unique"),
        ]
...
class Entitlement(models.Model):
    ...
    class Meta:
        ordering = ["-created_at"]
        constraints = [
            models.UniqueConstraint(fields=["provider", "provider_reference"], name="billing_entitlement_provider_reference_unique"),
        ]
        indexes = [models.Index(fields=["tenant", "active", "expires_at"])]

    def __str__(self) -> str:
        return f"{self.tenant} - {self.price.product} until {self.expires_at.isoformat()}"
