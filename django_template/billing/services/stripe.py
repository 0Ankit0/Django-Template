...
from ..models import BillingCustomer, CheckoutSession, Invoice, Payment, Price, Product, Provider, Subscription, WebhookEvent
...
def create_checkout_session(request, price: Price) -> CheckoutSession:
    ...
    return CheckoutSession.objects.create(tenant=tenant, price=price, provider=Provider.STRIPE, provider_session_id=data["id"], mode=mode, status=data.get("status", "open"), url=data.get("url", ""), metadata=data.get("metadata") or metadata)
...
def create_portal_session(request) -> str:
    ...
    data = _stripe_dict(_stripe_client().v1.billing_portal.sessions.create({"customer": customer.provider_customer_id, "return_url": request.build_absolute_uri(reverse("billing:dashboard"))}))
    return str(data["url"])
...
def cancel_subscription(subscription: Subscription, at_period_end: bool = True) -> Subscription:
    ...
    data = _stripe_dict(_stripe_client().v1.subscriptions.update(subscription.provider_subscription_id, {"cancel_at_period_end": at_period_end}))
...
def sync_invoice(data: dict[str, Any]) -> Invoice:
    ...
    invoice, _ = Invoice.objects.get_or_create(provider=Provider.STRIPE, provider_invoice_id=data["id"], defaults={"tenant": customer.tenant, "subscription": subscription, "status": data.get("status") or Invoice.Status.DRAFT})
