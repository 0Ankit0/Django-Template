        ...
        with transaction.atomic():
            ...
            if event_type == "checkout.session.completed":
                ...
                if session:
                    ...
                    if session.mode == CheckoutSession.Mode.PAYMENT and data.get("payment_intent") and data.get("payment_status") == "paid" and session.price.is_one_time:
                ...
                if session and data.get("payment_intent"):
                    payment = sync_payment_intent({"id": str(data["payment_intent"]), "customer": data.get("customer"), "amount": data.get("amount_total"), "currency": data.get("currency"), "status": "succeeded", "metadata": data.get("metadata") or session.metadata})
                ...
                if payment_intent:
                    status = Payment.Status.SUCCEEDED if event_type in {"invoice.paid", "invoice.payment_succeeded"} else Payment.Status.FAILED if event_type == "invoice.payment_failed" else Payment.Status.PENDING
                    Payment.objects.update_or_create(provider=Provider.STRIPE, provider_payment_id=str(payment_intent), defaults={"tenant": invoice.tenant, "subscription": invoice.subscription, "amount": int(data.get("amount_paid") or data.get("amount_due") or 0), "currency": str(data.get("currency") or "usd").upper(), "status": status, "provider_invoice_id": data["id"], "paid_at": timezone.now() if status == Payment.Status.SUCCEEDED else None, "metadata": data.get("metadata") or {}})
